...
n_T = 50
net_type = "fc"
extra_diffusion_steps = 16 # "X"

# get datasets
tf = transforms.Compose([])
torch_data_train = ClawCustomDataset(DATASET_PATH, transform=tf, train_or_test="train", train_prop=0.90)
dataload_train = DataLoader(torch_data_train, batch_size=batch_size, shuffle=True, num_workers=0)

x_shape = torch_data_train.image_all.shape[1:]
y_dim = torch_data_train.action_all.shape[1]

# set up model
nn_model = Model_cnn_mlp(x_shape, n_hidden, y_dim, embed_dim=128, net_type=net_type).to(device)
model = Model_Cond_Diffusion(nn_model, betas=(1e-4, 0.02), n_T=n_T, device=device, x_dim=x_shape, y_dim=y_dim)
model.to(device)
...
for i, idx in enumerate(idxs):
    x_eval = torch.Tensor(torch_data_train.image_all[idx]).type(torch.FloatTensor).to(device)
    ...
    axs[2,i].scatter(y_pred[:, 0] * 64, y_pred[:, 1] * 64, **SCATTER_KWARGS)
    axs[2,i].set_xticks([])
    axs[2,i].set_yticks([])
